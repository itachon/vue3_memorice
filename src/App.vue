...
const selc = document.querySelectorAll.bind(document)
const selo = document.querySelector.bind(document)

//COMPOSABLE
import { asignado } from './composables/functions.js'
import { apiCards } from './composables/api.js'
const { apicards, carts } = apiCards()
const { asignar, seleccionadas, points, errors, iniciar, restart } = asignado(selc, selo, carts);
...
onMounted(() => {
    apicards()
    selo('#inicio').classList.remove('hidden')
})
</script>